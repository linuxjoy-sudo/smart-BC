...
| 25 | 语音全双工改造（流式 ASR sherpa-onnx + 流式 TTS Qwen3-TTS + WebRTC AEC + 打断 + 连续对话） | 🔄 进行中 | `55e2c5f` `a50a796` | **Phase 1 完成**：sherpa-onnx 流式模块（paraformer int8）+ Windows 构建（MD 库解 CRT 冲突）；待：流式集成实测 → Phase 2 Qwen3-TTS → Phase 3 AEC → Phase 4 状态机 |
